...
(function () {
    ...
    var Module = angular.module('rmDatepicker', []);//달력 원본 복사??
    ...
    Module.directive("rmDatepicker", ['rmDatepickerConfig', '$compile', '$filter', '$document',
        ...
        var link = function (scope, element, attrs, ngModel) {
            var conf = angular.copy(rmDatepickerConfig); //달력 원본 복사
            if (scope.rmConfig) {
                for (var prop in conf)
                    if (conf.hasOwnProperty(prop))
                        if (scope.rmConfig[prop] != undefined) conf[prop] = scope.rmConfig[prop];
            }
            /*if (conf.min) conf.min = new Date('2020-03-19');*/
            ...
            scope.isOff = function (oDate) { //날짜비활성화??
            	//포맷하는부분
            	for(var i=0;i<FULLDATE.length;i++){
            	FULLDATE[i] = new Date(FULLDATE[i]);
                if (FULLDATE[i].toDateString() == oDate.toDateString())
                    return true;
            	}
        ...
        '<script type="text/ng-template" id="rm-month.html">' +
            ...
            '<div class="ng-class: state; square date">' +
            /*OFF: 꺼진것./OUT*/
/*ng-repeat :for문과 같다*/'<div ng-repeat="oDate in aDates" ng-class="{j: isActive[\'date\'](oDate), off: isOff(oDate), out: !isActive[\'month\'](oDate)}">' +
